...
void loop() {
  ...
  temp=2; //Definimos el delay como 5 ms   Nota: En algunos ordenadores, este tiempo se debe aumentar para que se vean todos los dígitos

  //Iniciamos un bucle infinito para leer el valor del potenciometro y encender sus dígitos en los display
  while(1){  
    voltaje=analogRead(pot); //Leemos el valor del potenciometro de 0 a 1023

    //Separamos el valor de voltaje en digitos mediante divisiones y modulo
    millares=voltaje/1000;
    centenas=(voltaje/100)%10; 
    ...
    digitalWrite(9,LOW); //Se enciende el pin 9 que satura la señal del transistor conectado a él, lo que permite el paso de señal al display unidades
    ...
    digitalWrite(9,HIGH); //Se apaga el pin 9, lo que desatura el transistor conectado a él, lo que apaga el display unidades
