...
void loop() {
  ...
  if(leftValue>Umbral && centerValue>Umbral && rightValue>Umbral){ //El robot está dentro de la plataforma
    avanzar();
  } 
  else{ //El robot está a punto de salirse o ya está fuera de la plataforma
    detener();  //Primero detiene al robot
    if(centerValue<Umbral) { //Está a punto de salirse por detras, avanza para reincorporarse
      avanzar();
    } 
    else { //Está a punto de salirse por los lados
      girar();
    }
  }
  ...
  if (distancia<30){  //Oponente detectado: atacar
    avanzar();
  } 
  else { // No hay oponente: buscarlo
    girar();
  }
}
...
// Funciones adicionales para las nuevas acciones
void retroceder() {
  motor1.setSpeed(200);
  motor2.setSpeed(200);
  motor1.run(BACKWARD);
